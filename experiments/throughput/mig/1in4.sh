...
for ((i = tested_comb; i < comb_len; i++)); do
  python main.py --task serve --model_num 1 --comb ${combination["$i"]} --policy EDF --load 25 --qos ${qos_target["$i"]} --queries 250 --thld 5 --ways 2 --abandon --mig 4
done

for ((i = tested_comb; i < comb_len; i++)); do
  python main.py --task serve --model_num 1 --comb ${combination["$i"]} --policy FCFS --load 25 --qos ${qos_target["$i"]} --queries 250 --thld 5 --ways 2 --abandon --mig 4
done
