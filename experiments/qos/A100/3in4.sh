...
for ((i = tested_comb; i < comb_len; i++)); do
  python main.py --task serve --model_num 3 --comb ${combination["$i"]} --policy SJF --load 50 --qos ${qos_target["$i"]} --queries 1000 --thld 5 --ways 2 --abandon
done

for ((i = tested_comb; i < comb_len; i++)); do
  python main.py --task serve --model_num 3 --comb ${combination["$i"]} --policy FCFS --load 50 --qos ${qos_target["$i"]} --queries 1000 --thld 5 --ways 2 --abandon
done

for ((i = tested_comb; i < comb_len; i++)); do
  python main.py --task serve --model_num 3 --comb ${combination["$i"]} --policy EDF --load 50 --qos ${qos_target["$i"]} --queries 1000 --thld 5 --ways 2 --abandon
done

for ((i = tested_comb; i < comb_len; i++)); do
  python main.py --task serve --model_num 3 --comb ${combination["$i"]} --policy Abacus --load 50 --qos ${qos_target["$i"]} --queries 1000 --thld 5 --ways 2 --abandon
done
